...
if (-not $SkipAzecotronRuntime) {
  ...
  $fallbackManifest | ConvertTo-Json -Depth 4 | Set-Content (Join-Path $resource "runtime-manifest.json")
}
...
Step "Build NSIS and MSI installers" {
  cargo tauri build --manifest-path "$root\src-tauri\Cargo.toml" --bundles nsis,msi
}
...
$artifacts = Get-ChildItem $bundle -Recurse -File -Include *.exe,*.msi
if (-not $artifacts) { throw ("No installer artifacts found under " + $bundle) }

$artifacts | ForEach-Object { Copy-Item $_.FullName $stable -Force }
...
$installerManifest = [ordered]@{
  product = "Synth Browser"
  version = "0.1.0"
  runtime = if ($SkipAzecotronRuntime) { "Tauri WebView2" } else { "Azecotron Web" }
  azecotron = if ($SkipAzecotronRuntime) { "NOT_BUNDLED" } else { "BUNDLED" }
  chromium = if ($SkipAzecotronRuntime) { $null } else { "152.0.7977.119" }
  installers = $outputs
}
