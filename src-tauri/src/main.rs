...
#[tauri::command]
fn print_page_to_pdf(app: tauri::AppHandle, state: State<AppState>)->AppResult<String>{
    let id=state.active_id.lock().unwrap().clone();
    let view=app.get_webview(&format!("page-{id}")).ok_or_else(||AppError::Message("No active web page.".into()))?;
    let dir=dirs_next::download_dir().unwrap_or_else(||PathBuf::from(".")).join("Synth Browser").join("pdf");
...
fn sha256_file(path:&Path)->AppResult<Option<String>>{
    if !path.is_file(){return Ok(None)}
    let mut file=fs::File::open(path)?;
    let mut hasher=Sha256::new();
    let mut buf=[0u8;1024*1024];
    loop{let n=std::io::Read::read(&mut file,&mut buf)?;if n==0{break}hasher.update(&buf[..n]);}
    Ok(Some(format!("{:x}",hasher.finalize())))
}

#[tauri::command]
fn profile_integrity(state: State<AppState>)->AppResult<serde_json::Value>{
    let root=profile_dir(&state.profile.id);
    Ok(serde_json::json!({
      "format":"synth-profile-integrity-v1",
      "profile_id":state.profile.id,
      "profile_name":state.profile.name,
      "database_present":root.join("browser.sqlite3").is_file(),
      "database_sha256":sha256_file(&root.join("browser.sqlite3"))?,
      "extension_count":state.db.list_extensions()?.len()
    }))
}

#[tauri::command]
fn verify_profile_integrity(state: State<AppState>, expected_db_sha256:Option<String>)->AppResult<serde_json::Value>{
    let actual=sha256_file(&profile_dir(&state.profile.id).join("browser.sqlite3"))?;
    let verified=match (&expected_db_sha256,&actual){
...
fn export_profile(state: State<AppState>)->AppResult<String>{
    ...
    let manifest=serde_json::json!({
        ...
        "exported_at":Db::now(),
        "integrity":{"database_sha256":sha256_file(&source.join("browser.sqlite3"))?}
    });
...
#[tauri::command]
fn schedule_update_after_exit(state: State<AppState>, staged_path:String, version:String, sha256:String)->AppResult<String>{
    let staged=PathBuf::from(&staged_path);
    let current=std::env::current_exe()?;
    let root=app_root().join("updates");
    let pending=updater::prepare_verified_swap(&current,&staged,&version,&sha256,&root).map_err(AppError::Message)?;
    let pending_path=updater::write_pending(&root,&pending).map_err(AppError::Message)?;
    state.db.set_setting("update_pending","true")?;
...
fn main() {
    ...
    let app = tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            complete_onboarding, privacy_preset, get_settings, set_setting, reset_settings, verify_update_manifest, update_status, updater_staging_path, fetch_update_manifest, stage_verified_update, schedule_verified_update, schedule_update_after_exit, pending_update, clear_pending_update, update_rollback_backup,
